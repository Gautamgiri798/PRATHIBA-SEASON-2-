...
const totalVoters = countArg ? parseInt(countArg, 10) : 10;

// Nominees requested by the user
const VOTES_MAP = {
  "best-actor-music": "n1",                 // Ankit Pattnaik
  ...
  "best-actor/actress-in-comedy": "n3"      // Lokesh
};
...
async function run() {
  ...
  console.log("Casting votes for nominees:");
  console.table(VOTES_MAP);
...
async function runSqlite() {
  ...
  for (let i = 0; i < totalVoters; i++) {
    ...
    try {
      ...
      for (const [catId, nomId] of Object.entries(VOTES_MAP)) {
  ...
  console.log("\n🎉 Done! All 10 voters successfully processed in SQLite test database.");
}
...
async function runPostgres() {
  ...
  await pool.end();
  console.log("\n🎉 Done! All 10 voters successfully processed in PostgreSQL production database.");
}
